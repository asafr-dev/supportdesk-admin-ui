...
export function TicketsPage() {
  ...
  const statusParsed = status ? TicketStatus.safeParse(status) : null;
  const statusForApi = statusParsed?.success ? statusParsed.data : undefined;
  const { limit, offset } = clampPagination({
    limit: sp.get("limit") === null ? undefined : Number(sp.get("limit")),
    offset: sp.get("offset") === null ? undefined : Number(sp.get("offset"))
  });
  ...
  const title = useMemo(() => {
    const parts = [];
    if (status) {
      parts.push(statusForApi ? ticketStatusLabel(statusForApi) : status);
    }
    if (qDebounced) parts.push(`q="${qDebounced}"`);
    return parts.length ? `Tickets (${parts.join(", ")})` : "Tickets";
  }, [status, statusForApi, qDebounced]);
